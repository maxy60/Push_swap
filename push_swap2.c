...
/*   Updated: 2021/12/04 13:17:19 by msainton         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	pos_value_a(t_stack *stack, int nbr)
{
	...
	stack = stack->next;
	i++;
	while (stack && !(nbr < stack->element && nbr > stack->prec->element)
		&& nbr > min_value(stack))
	{
		i++;
		stack = stack->next;
...
int	calc_a(t_stack *stack_a, int nbr)
{
	...
	count++;
	while (stack_a && !(nbr < stack_a->element && nbr > stack_a->prec->element)
		&& nbr > min_value(stack_a))
	{
		count++;
		stack_a = stack_a->next;
	}
...
t_mynbr	get_min_op(t_stack *stack_a, t_stack *stack_b)
{
	...
	{
		tmp_nbr = tmp->element;
		tmp_count = calc_b(stack_b, tmp->element)
			+ calc_a(stack_a, tmp->element);
